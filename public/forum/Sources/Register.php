...
function Register($reg_errors = array())
{
	...
	// initialize notCaptcha
	require_once($sourcedir."/not-captcha.php");
...
function Register2($verifiedOpenID = false)
{
	...
	if (!$verifiedOpenID)
	{
		...
		// was notCaptcha solved correctly?
		require_once($sourcedir."/not-captcha.php");
		$notCaptcha_reg_error = notCaptcha_CheckRegisterPostNew();
		if (!empty($notCaptcha_reg_error))
			$reg_errors[] = $txt[$notCaptcha_reg_error];
		unset($notCaptcha_reg_error);
	...
	// do some cleanup for notCaptcha
	if (function_exists('notCaptcha_RegistrationDone'))
...
// generate notCaptcha image
function notCaptcha_img() {
	/*
	...
	*/
	...
	require_once($boarddir.'/notcaptcha/notcaptcha_config.php');

	if (isset($_SESSION['nc_session']))
	...
	if($_REQUEST['i'] == 3 OR $_REQUEST['i'] == 2 OR $_REQUEST['i'] == 1)
	...
	$angles = $modSettings['notCaptcha']['notcaptcha_angles'];

	// load images from gallery
	$images = array();
	$imgdir_absolute = $boarddir.'/'.$modSettings['notCaptcha']['notcaptcha_imgdir'];
	...
	// find not used image
	if (isset($_SESSION['nc_used_images'])) {
	...
	$background	=	imagecolorallocate($img, $modSettings['notCaptcha']['nc_back_r'], $modSettings['notCaptcha']['nc_back_g'], $modSettings['notCaptcha']['nc_back_b']);
	$delim		=	imagecolorallocate($img, $modSettings['notCaptcha']['notcaptcha_delim_color'][0], $modSettings['notCaptcha']['notcaptcha_delim_color'][1], $modSettings['notCaptcha']['notcaptcha_delim_color'][2]);
	...
	// save correct answer
	$_SESSION['nc_tries'] = 0;
	$cookie_encrypt = md5(session_id().array_search(0, $angles, 1).$modSettings['notCaptcha']['nc_cookie_enc']);
	setcookie('nc_answer_'.$imgNum, $cookie_encrypt);

	$pos = 0;
	$counter = 1;
	$imgRes=imagecreatetruecolor($modSettings['notCaptcha']['imagesize']*count($modSettings['notCaptcha']['notcaptcha_angles'])+count($modSettings['notCaptcha']['notcaptcha_angles'])-1, $modSettings['notCaptcha']['imagesize']);
	foreach ($angles as $angle) {
		$img2 = imagerotate( $img, $angle, $background );
		$width = imagesx($img2);
		$height = imagesy($img2);
		imagecopyresampled( $imgRes, $img2, $pos, 0, abs(round(($size - $width)/2)), abs(round(($size - $height)/2)), $modSettings['notCaptcha']['imagesize'], $modSettings['notCaptcha']['imagesize'], $size, $size);
		imageline( $imgRes, $pos+$modSettings['notCaptcha']['imagesize'], 1, $pos+$modSettings['notCaptcha']['imagesize'], $modSettings['notCaptcha']['imagesize'], $delim);

		if ($modSettings['notCaptcha']['nc_addnoise'] == 'true') {
			// noise block
			$dx = rand(0, $modSettings['notCaptcha']['imagesize'] / 3);
			$dw = round($dx + rand(0, $modSettings['notCaptcha']['imagesize'] / 3));
			if (rand(1,10) > 5) $dx = -$dx;
			$dy = rand(0, $modSettings['notCaptcha']['imagesize'] / 2);
			$dh = round($dy + rand(0, $modSettings['notCaptcha']['imagesize'] / 2));
			if (rand(1,10) > 5) $dy = -$dy;
			imagefilledellipse ( $imgRes , $pos + round($modSettings['notCaptcha']['imagesize']/2) + $dx , round($modSettings['notCaptcha']['imagesize']/2) + $dy , $dw , $dh, imagecolorallocatealpha($imgRes, rand(100,250), rand(100,250), rand(100,250), rand(30, 80)));
			// end of noise block
		} // nc_addnoise
		...
		$pos += $modSettings['notCaptcha']['imagesize'] + 1;
	} // foreach

	
	if(function_exists("imagejpeg")){
		header("Content-Type: image/jpeg");
		imagejpeg($imgRes, null, $modSettings['notCaptcha']['notcaptcha_jpeg_quality']);
